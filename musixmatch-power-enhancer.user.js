...
(function () {
  ...
  function resetInjectedState(removePanel = true) {
    if (removePanel) {
      ...
      document.querySelectorAll(`[${LYRICS_STATS_COLUMN_ATTR}]`).forEach((el) => {
  ...
  function createNativeLyricsStatChip(svgPath, text) {
    ...
    const row = createEl('div', { className: 'css-g5y9jx r-1awozwy r-18u37iz' });
  ...
  function renderCuratorsBox(track, targetContainer = null) {
    ...
    appleTab.addEventListener('click', () => renderService('itunes'));
    renderService('spotify');
    function makeFooterRow(labelText, valueText, accentColor, withExportBtn) {
      ...
      const val = createEl('span', { text: valueText });
  ...
  function importDrafts(file) {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (hasConflict) {
          if (!window.confirm("Some of the loaded drafts already exist in the browser. Do you want to overwrite them?")) {
            return;
          }
        }
        ...
        window.alert("Drafts loaded successfully! The page will refresh in 5 seconds.");
        setTimeout(() => {
  ...
  function injectDraftsIcon() {
    ...
    const coverWrapper = document.querySelector('.css-g5y9jx.r-1awozwy.r-izgom.r-1xfd6ze.r-h3s6tt.r-1777fci.r-1udh08x.r-rwqe4o');
    ...
    container.style.cssText = 'position:relative;display:flex;align-items:center;margin-right:12px;z-index:999;';
    ...
    iconWrap.onmouseover = () => innerCircle.style.background = 'rgba(255,255,255,0.1)';
    iconWrap.onmouseout = () => innerCircle.style.background = 'transparent';
    const menu = createEl('div', {
      className: 'mxm-ext-panel',
    });
    menu.style.cssText = 'position:absolute;top:100%;left:0;margin-top:8px;display:none;flex-direction:column;min-width:180px;gap:4px;padding:6px;box-shadow:0 10px 30px rgba(0,0,0,0.8);background:#1a1a1a;border:1px solid rgba(255,255,255,0.1);border-radius:12px;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;';
    const btnStyle = 'background:transparent;border:none;color:var(--mxm-contentPrimary, #fff);padding:10px 14px;text-align:left;cursor:pointer;border-radius:8px;width:100%;font-size:13px;font-family:inherit;font-weight:500;white-space:nowrap;transition:background 0.15s;';
    ...
    exportBtn.onmouseover = () => exportBtn.style.background = 'rgba(255,255,255,0.1)';
    ...
    importBtn.onmouseover = () => importBtn.style.background = 'rgba(255,255,255,0.1)';
    importBtn.onmouseout = () => importBtn.style.background = 'transparent';
    importBtn.onclick = () => {
      menu.style.display = 'none';
      fileInput.click();
    };
    const fileInput = createEl('input', { attrs: { type: 'file', accept: '.json' }});
